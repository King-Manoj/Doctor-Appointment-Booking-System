...
const MyAppointments = () => {
  ...
  return (
    <div>
       <p className='pb-3 mt-12 font-semibold text-xl text-zinc-700 border-b'>My Appointments</p>
       <div>
        {
          doctors.slice(0,3).map((item,index) => (
            <div className='grid grid-cols-[1fr_2fr] sm:flex gap-4 sm:gap-6 py-2 border-b' key={index}>
              ...
              <div className='flex-1 text-sm text-zinc-600'>
                ...
                <p>{item.speciality}</p>
                <p className='text-zinc-800 font-medium mt-1'>Adress:</p>
                <p className='text-xs'>{item.address.line1}</p>
                <p className='text-xs'>{item.address.line2}</p>
                <p className='text-xs mt-1'><span className='text-sm text-neutral-800 font-medium'>Date & Time:</span>25, Dec, 2024 | 8.30PM</p>
              </div>
              <div></div>
